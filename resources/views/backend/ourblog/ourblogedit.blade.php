...
<img class="rounded-circle" src="{{ asset('img/user.jpg') }}" alt="" style="width: 40px; height: 40px;">
            <div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
             ...
             <div class="navbar-nav w-100">
                    ...
                    <a href="{{ route('all.ourblog') }}" class="nav-item nav-link active "><i class="fa fa-chart-bar me-2"></i>Our blog</a>
        ...
        <div class="content">
            ...
            <nav class="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0">
                ...
                <form action="{{ route('ourblog.update' , $ourblogs->id) }}" method="POST" enctype="multipart/form-data" class="col-12 mt-5">
                       @csrf

               <div class="container">
                 <center>
                 <h1>our blog update</h1>
                 </center><br><br>
     ...
     <div class="col-10">
                  ...
                  <input type="text" class="form-control" name="name" placeholder="Your product site name" value="{{ $ourblogs->name }}"><br>
                @error('name')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror

             <input type="text" class="form-control" name="title" placeholder="Your product site title" value="{{ $ourblogs->title }}"><br>
                @error('title')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
