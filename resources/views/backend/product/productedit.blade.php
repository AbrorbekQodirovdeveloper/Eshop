    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            ...
            <div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
                 ...
                 <div class="navbar-nav w-100">
                    ...
                    <a href="{{ route('all.offer') }}" class="nav-item nav-link"><i class="fa fa-table me-2"></i>Offer</a>
                    <a href="{{ route('all.category') }}" class="nav-item nav-link"><i class="fa fa-table me-2"></i>Category</a>
                    <a href="{{ route('all.product') }}" class="nav-item nav-link active"><i class="fa fa-chart-bar me-2"></i>Product</a>
        ...
        <div class="content">
            ...
            <nav class="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0">
                ...
                <form action="{{ route('product.update' , $products->id) }}" method="POST" enctype="multipart/form-data" class="col-12 mt-5">
                       @csrf
     ...
     <div class="col-10">
                ...
                <input type="number" class="form-control" name="category_id" placeholder="Your product site category id" value="{{ $products->category_id }}"><br>
                @error('category_id')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                  <input type="text" class="form-control" name="name" placeholder="Your product site name" value="{{ $products->name }}"><br>
                @error('name')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
               <input type="number" class="form-control" name="price" placeholder="Your product site price" value="{{ $products->price }}"><br>
                @error('price')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                 <input type="number" class="form-control" name="oldprice" placeholder="Your product site oldprice" value="{{ $products->oldprice }}"><br>
                @error('oldprice')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                <input type="number" class="form-control" name="discount" placeholder="Your product site discount" value="{{ $products->discount }}"><br>
                @error('discount')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
             <input type="text" class="form-control" name="title" placeholder="Your product site title" value="{{ $products->title }}"><br>
                @error('title')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                <input type="text" class="form-control" name="about" placeholder="Your product site about" value="{{ $products->about }}"><br>
                @error('about')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                     <input type="text" class="form-control" name="type" placeholder="Your product site type" value="{{ $products->type }}"><br>
                @error('type')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
