    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            ...
            <div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
             ...
             <div class="navbar-nav w-100">
                    ...
                    <a href="{{ route('all.ourblog') }}" class="nav-item nav-link"><i class="fa fa-chart-bar me-2"></i>Our blog</a>
                    ...
                    <a href="{{ route('all.review') }}" class="nav-item nav-link active"><i class="fa fa-chart-bar me-2"></i>Review</a>
                </div>
            </nav>
        </div>
        <!-- Sidebar End -->
        ...
        <div class="content">
            ...
            <nav class="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0">
                ...
                <form action="{{ route('productsetsize.update' , $reviews->id) }}" method="POST" enctype="multipart/form-data" class="col-12 mt-5">
                       @csrf

               <div class="container">
                 <center>
                 <h1>Product set size update</h1>
                 </center><br><br>

         <div class="row">
     <div class="col-10">
                  <input type="number" class="form-control" name="product_id" placeholder="Your review product id" value="{{ $reviews->product_id }}"><br>
                @error('product_id')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                   <input type="number" class="form-control" name="client_id" placeholder="Your review client id" value="{{ $reviews->size_id }}"><br>
                 @error('client_id')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                      <input type="number" class="form-control" name="rating" placeholder="Your review rating " value="{{ $reviews->rating }}"><br>
                 @error('rating')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                  <input type="text" class="form-control" name="comment" placeholder="Your review comment " value="{{ $reviews->comment }}"><br>
                 @error('comment')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
