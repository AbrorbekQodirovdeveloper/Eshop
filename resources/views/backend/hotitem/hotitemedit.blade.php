    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            ...
            <div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
                 ...
                 <div class="navbar-nav w-100">
                    ...
                    <a href="{{ route('all.hotitem') }}" class="nav-item nav-link active"><i class="fa fa-chart-bar me-2"></i>Hotitem</a>
        ...
        <div class="content">
            ...
            <nav class="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0">
                ...
                <form action="{{ route('hotitem.update' , $hotitems->id) }}" method="POST" enctype="multipart/form-data" class="col-12 mt-5">
                       @csrf

               <div class="container">
                 <center>
                 <h1>hotitem update</h1>
                 </center><br><br>
     ...
     <div class="col-10">
                  ...
                  <input type="text" class="form-control" name="name" placeholder="Your product site name" value="{{ $hotitems->name }}"><br>
                @error('name')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
               <input type="number" class="form-control" name="price" placeholder="Your product site price" value="{{ $hotitems->price }}"><br>
                @error('price')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                 <input type="number" class="form-control" name="oldprice" placeholder="Your product site oldprice" value="{{ $hotitems->oldprice }}"><br>
                @error('oldprice')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                <input type="number" class="form-control" name="discount" placeholder="Your product site discount" value="{{ $hotitems->discount }}"><br>
                @error('discount')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
             <input type="text" class="form-control" name="title" placeholder="Your product site title" value="{{ $hotitems->title }}"><br>
                @error('title')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
                <input type="text" class="form-control" name="about" placeholder="Your product site about" value="{{ $hotitems->about }}"><br>
                @error('about')
                   <span class="text-danger">{{ $message }}</span>
                    @enderror
